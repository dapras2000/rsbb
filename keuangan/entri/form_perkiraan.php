...
if (isset($_SESSION['KDUNIT']))
{
	...
	$objTKNeraca->db_host=$hostname;
	$objTKNeraca->db_user=$username;
	$objTKNeraca->db_pass=$password;
	$objTKNeraca->db_database=$database; 	
	...
	if ($modus=='modify'){
		...
		$allFields=$objTKNeraca->getAllFieldsById($id,$tahun); 
...
$(document).ready(function(){
	$('#simpan').click(function(){
			...
			if ($('#modus').val()=='modify'){
				$.post('keuangan/entri/proses_neraca.php',$('#formData').serialize(),function(data){																												
					id=id.replace(/[.]+/g,'_');		
...
<form action="keuangan/entri/proses_neraca.php'" method="post" name="formData" id="formData" >
  <table>
     <tr>
         <td>Kode Akun </td> 
