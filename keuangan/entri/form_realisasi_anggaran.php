...
if (isset($_SESSION['KDUNIT']))
{
	...
	$namaakun='';
	$id='';
	require_once("../entri/tk_realisasi_anggaran.php");
	$objTKRLA = new tk_realisasi_anggaran();
	$objTKRLA->db_host=$hostname;
	$objTKRLA->db_user=$username;
	$objTKRLA->db_pass=$password;
	$objTKRLA->db_database=$database; 
	...
	if ($modus=='modify'){
		...
		$allFields=$objTKRLA->getAllFieldsById($id,$tahun); 
...
$(document).ready(function(){
	$('#simpan').click(function(){
			...
			if ($('#modus').val()=='modify'){
				$.post('keuangan/entri/proses_rla_bb.php',$('#formData').serialize(),function(data){																												
					id=id.replace(/[.]+/g,'_');		
...
<form action="keuangan/entri/proses_rla.php'" method="post" name="formData" id="formData" >
  <table>
     <tr>
         <td>Kode Akun </td> 
