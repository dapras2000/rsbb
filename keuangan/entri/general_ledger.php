  ...
  if (modus=='delete'){
		$(document).ready(function(){
			if ( confirm ('Yakin hapus '+name+' ?')==true){		   
				$.post('./entri/delete.php',{id: id},function(response){											
							var x=response.trim(' ');								
							if(x == 'ok'){
...
<form action="?<?php echo 'link=general_ledger' ?>" method="post" class="input_tahun" id="cari" name="cari">
...
<?php
	...
	mysql_query("CALL pr_eksekutif_total_pendapatan_rekap_bb('".$tahun."',1)");
	foreach($all_childs as $item) {
		$nilai=0;
		if ($item['slave'] == '1' ) {
			if($item['Id'] == '01.01.08'){
				 $query	= mysql_query('SELECT tanggal,SUM(manajemen) AS manajemen, SUM(rj) AS rj, SUM(ugd) AS ugd, SUM(vk) AS vk, SUM(lab) AS lab, SUM(rad) AS rad, SUM(ri) AS ri, SUM(ok) AS ok, SUM(apotek) AS apotek,SUM(gizi) AS gizi,
	SUM(vk_ri) AS vk_ri, SUM(lab_ri) AS lab_ri, SUM(rad_ri) AS rad_ri, SUM(apotek_ri) AS apotek_ri,SUM(gizi_ri) AS gizi_ri, carabayar
	FROM xxx_rekap_pendapatan_bulanan
	where carabayar = 1');
				 if(mysql_num_rows($query) > 0){
					$data	= mysql_fetch_array($query);
					$nilai	= $data['manajemen']+$data['rj']+$data['ugd']+$data['vk']+$data['lab']+$data['rad']+$data['gizi']+$data['apotek']+$data['ok']+$data['ri']+$data['vk_ri']+$data['lab_ri']+$data['rad_ri']+$data['gizi_ri']+$data['apotek_ri'];
				 }else{
					$nilai	= 0;
				 }
			}else if($item['Id'] == '01.01.09'){
				 $query2	= mysql_query('SELECT tanggal,SUM(manajemen) AS manajemen, SUM(rj) AS rj, SUM(ugd) AS ugd, SUM(vk) AS vk, SUM(lab) AS lab, SUM(rad) AS rad, SUM(ri) AS ri, SUM(ok) AS ok, SUM(apotek) AS apotek,SUM(gizi) AS gizi,
	SUM(vk_ri) AS vk_ri, SUM(lab_ri) AS lab_ri, SUM(rad_ri) AS rad_ri, SUM(apotek_ri) AS apotek_ri,SUM(gizi_ri) AS gizi_ri, carabayar
	FROM xxx_rekap_pendapatan_bulanan
	where carabayar > 1');
				...
				$strSQL="select * from tk_realisasi_anggaran where tahun='$tahun' and id='$item[Id]'";
...
<?php if ($item['slave'] == '1' ){?>
<a class='thickbox'  href="keuangan/entri/form_realisasi_anggaran.php?&modus=modify&id=<?php echo $item['Id']?>&hideid=<?php echo $item['Id']?>&tahun=<?php echo $tahun ?>&nilai=<?php echo $nilai?>  ">
