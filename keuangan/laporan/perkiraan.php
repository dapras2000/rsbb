...
?>
<form action="?<?php echo'link=35k1' ?>" method="post" class="input_tahun">
...
<?php
	...
	foreach($all_childs as $item) {
		...
		if ($item['slave'] == '1' ) {
			...
			FROM mk_neraca a
			LEFT JOIN tk_neraca b ON a.id=b.id AND b.tahun IN('$tahun')  
			LEFT JOIN tk_neraca c ON a.id=c.id AND c.tahun IN('$tahunNext')
			WHERE  a.id='$item[Id]' ";
