...
$search = '';
$date2 = date('Y-m-d');
$tanggalobat = mysql_query ("select TANGGAL from t_pemberianobat 
						left join t_admission on t_pemberianobat.IDXRANAP = t_admission.id_admission 
						where date_format(t_pemberianobat.TANGGAL,'%Y-%m-%d') = '".$date2."'");
if(mysql_num_rows($tanggalobat) >0){
	$search = ' AND (DATE_FORMAT(f.TANGGAL,"%Y/%m/%d") BETWEEN "'.$start.'" and "'.$end.'")';
}else{
	$search = ' AND (DATE_FORMAT(a.masukrs,"%Y/%m/%d") BETWEEN "'.$start.'" and "'.$end.'")';
}
$norm = "";
if(!empty($_GET['norm'])) {
    $norm =$_GET['norm'];
...
<div align="center">
    <div id="frame" style="width: 100%;">
        <div id="frame_title"><h3>LIST OBAT PASIEN RANAP</h3></div>
    ...
    echo $nama;
                    ...
                    <tr>
                        ...
                        <td><input type="submit" value="Cari" class="text"/>
                            <input type="hidden" name="link" value="list_pasien_apotek_ranap" /></td>
                    </tr>
                </table>
            ...
            <div id="table_search">
				...
				<tr align="center"><th>No</th><th>TGL Masuk</th><th>No RM</th><th>Nama Pasien</th><th>Alamat</th><th>Poli/ Ruang</th><th>Dokter Pengirim</th><th width="70px">Cara Bayar</th><th width="200px">&nbsp;</th></tr>
                    <?
                    $NO=0;
                    $sql = 'SELECT a.nomr, a.NIP, b.NAMA, b.ALAMAT, c.NAMA AS CARABAYAR, a.masukrs,a.noruang, a.nott,a.id_admission, d.nama AS namaruang, e.NAMADOKTER, a.id_admission 
							FROM t_admission a 
							...
							LEFT JOIN t_pemberianobat f ON f.IDXRANAP = a.id_admission
							WHERE a.noruang != 0 AND (a.keluarrs IS NULL OR DATE_FORMAT(a.keluarrs,"%Y/%m/%d") = "0000/00/00")
							'.$search.' group by a.nomr order by a.masukrs asc';
                    $pager = new PS_Pagination($connect, $sql, 15, 5, "tgl_kunjungan=".$tgl_kunjungan."&nama=".$nama."&norm=".$norm,"index.php?link=list_pasien_apotek_ranap&");
                    $rs = $pager->paginate();
                    ...
                    <?php
                        ...
                        <td><?php echo $data['namaruang'].' / '.$data['nott'];?></td>
                        <td><? echo $data['NAMADOKTER']; ?></td>
                        <td><? echo $data['CARABAYAR']; ?></td>
                        <td>
						<?php
							echo '<a href="'._BASE_.'index.php?link=add_resep_ranap&nomr='.$data['nomr'].'&idx='.$data['id_admission'].'&rajal=0"><input type="button" value="PROSESS" class="text"/></a>';
						?>
                        </td>
                    </tr>
                        <?	$i++;
