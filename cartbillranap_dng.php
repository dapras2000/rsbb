...
jQuery(document).ready(function(){
	...
	jQuery('.cancel').click(function(){
		...
		var idxbill	= jQuery(this).attr('idxbill');
		var idxdaftar = jQuery(this).attr('idxdaftar');
		//cancel
		jQuery.post('<?php echo _BASE_;?>cartbill_pembayaran_batal_ranap.php',{idxbill:idxbill,nobill:nobill,nomr:nomr,kode:kode,idxdaftar:idxdaftar},function(data){
			...
			w.close();
			location.reload();
			//window.location.href('<?php echo _BASE_; ?>carttambahdeposit.php?idxb=172754&t='<?php echo $userdata['TOTTARIFRS']; ?>);
		});
	});
...
<tr><td>No MR</td><td><?php echo $userdata['nomr'];?>
          	...
          	<span style="float:right"><a href="index.php?link=34tb&idxb=<?=$_GET['idxb']?>&t=<?=$a2?>"><input type="button" value="Tambah Deposit" class="text"/></a></span>
</td></tr>
...
<table width="95%" border="0" cellpadding="0" cellspacing="0" class="tb">
  <tr>
    ...
    <?php
  $sql = "SELECT DISTINCT a.nama_tindakan AS nama_jasa, b.IDXBILL, b.CARABAYAR as kdcarabayar, h.STATUS,b.IDXBILL,
DATE_FORMAT(b.TANGGAL,'%d/%m/%Y') AS TGL1, b.qty, b.TARIFRS, c.NAMA as carabayar, d.NAMADOKTER,b.nobill,b.idxdaftar,a.kode_tindakan
FROM t_billranap b 
join t_bayarranap h on b.NOBILL = h.NOBILL
...
JOIN t_billrajal tb on tb.IDXDAFTAR=b.IDXDAFTAR
JOIN m_tarif2012 x on x.kode_tindakan=tb.KODETARIF
WHERE b.idxdaftar='".$_REQUEST['idxb']."' and b.nobill='".$_REQUEST['nobill']."' and b.STATUS !='BATAL'";
  ...
  ?>
    ...
    <td align="center"><? echo "Rp. ".curformat($data['TARIFRS']); ?></td>
    
    <td align="center"><? echo $data['qty']; ?></td>
    <td align="center"><? echo "Rp. ".curformat($data['TARIFRS']*$data['qty']); ?></td>
	...
	<td align="center">
		...
		<input type="button" name="Cancel" value="Batal" class="text cancel" idxdaftar="<?php echo $data['idxdaftar'];?>" idxbill="<?php echo $data['IDXBILL'];?>" kode="<?php echo $data['kode_tindakan'];?>" id="cancel_<?php echo $data['nobill']; ?>" rel="<?php echo $data['nobill']; ?>" svn="<?php echo $data['nobill']; ?>" />
		<?php
		}?>
	</td>
	</tr>
    ...
    <tr>
                    ...
                    <?php #endif; ?></td>
					<td align="right" style="background:#999; font-weight:bold; text-align:right; padding-right:10px;"></td>
                </tr>
