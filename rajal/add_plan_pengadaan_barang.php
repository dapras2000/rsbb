...
$sql = "SELECT temp_cartbarang.IDXBARANG AS IDX,
						...
						WHERE temp_cartbarang.IP = '$ip' AND  temp_cartbarang.KATEGORI = 'GP' 
		UNION
		SELECT temp_cartbarang.IDXBARANG AS IDX,
						...
						WHERE temp_cartbarang.IP = '$ip' AND  temp_cartbarang.KATEGORI = 'LP'";
...
?>
       <tr>
		 ...
		 ?></td>
         <td><a href="#" onclick="javascript: MyAjaxRequest('validbarang','rajal/add_plan_pengadaan_barang.php?optbarang=<?=$data['IDX']?>'); return false;" >Hapus</a></td>
      </tr>
<?php $i++; } ?>      
   </table>

   <form name="savebarang" id="savebarang" action="rajal/save_plan_pengadaan_barang.php" method="post" >
   <input type="hidden" name="bulan" value="<?=$bulan?>" />
   <input type="hidden" name="tahun" value="<?=$tahun?>" />
   <input class="text" type="submit" value="S i m p a n" onclick="submitform (document.getElementById('savebarang'),'rajal/save_plan_pengadaan_barang.php','validbarang',validatetask); return false;"/>
   </form>
   </fieldset>   
 <? } ?>
