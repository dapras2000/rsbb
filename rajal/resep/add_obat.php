...
if(empty($_POST['nm_barang2'])){
	echo "<strong>Maaf Obat Belum Diisi</strong>";
}else{
...
<table width="100%" class="tb" border="0" cellpadding="1" cellspacing="1">
  ...
  <tr>
    <td><?=$VIEW['NAMAOBAT']?></td>
    <td><?=$VIEW['SEDIAAN']?></td>
    <td><?=$VIEW['ATURANPAKAI']?></td>
    <td><?=$VIEW['JUMLAH']?></td>
    <td><a href="#" onclick="javascript:delobat('<?=$VIEW['IDXTEMP']?>');"><div style='padding:2px; background:#FFC; border:1px solid #CCC; margin:2px;' align='center'>remove</div></a></td>
  </tr>
  <? } ?>
</table>
