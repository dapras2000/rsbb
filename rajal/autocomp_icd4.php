		...
		if (mysql_num_rows ($userquery) > 0){
			...
			<?php
				while ($userdata = mysql_fetch_array ($userquery)){
					?><div onmouseover="this.style.background = '#EEEEEE'" onmouseout="this.style.background = '#CCCCCC'" onclick="setvalue_icd4 ('<?php echo $userdata['jenis_penyakit'];?>','<?php echo $userdata['icd_code']; ?>')"><?php echo $userdata['jenis_penyakit']; ?></div><?php
				}
			?>
			</div>
			<?php
