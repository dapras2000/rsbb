...
<div align="center">
    <div id="frame">
    <div id="frame_title">
      <h3>TAGIHAN KONSULTASI GIZI RAWAT JALAN/ HARI INI</h3></div>
...
<!--             <option value="3" <?php if($_GET['crbayar'] == 3): echo 'selected="selected"'; endif; ?>>JAMKESMAS</option>
      ...
      <input type="hidden" name="link" value="33gizi_rajal" />
...
WHERE a.KODETARIF LIKE '01.01.18%'".$kondisi." GROUP BY a.IDXDAFTAR";
	...
	$NO=0;
	$pager = new PS_Pagebill($connect, $sql, 15, 5, "start=".$_GET['start']."&end=".$_GET['end']."&poly=".$_GET['poly']."&cnama=".$_GET['cnama']."&cnomr=".$_GET['cnomr']."&crbayar=".$_GET['crbayar'],"index.php?link=33gizi_rajal&");
	$rs = $pager->paginate();
	...
	while($data = mysql_fetch_array($rs)) {
        ...
        ?>
              ...
              <td><a href="index.php?link=34gizi_rajal&nomr=<?php echo $data['NOMR']; ?>&poly=<?php echo $data['KDPOLY'];?>&idxdaftar=<?php echo $data['IDXDAFTAR'];?>"> <input type="button" class="text" value="Prosess" /></a> <!--<a href="index.php?link=33batal&idxb=<? echo $data['NOBILL'];?>&idxdaftar=<? echo $data['IDXDAFTAR'];?>"><input type="button" class="text" value="Batal" /></a>--></td>
            </tr>
            <?	} 
