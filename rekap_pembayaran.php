...
?>" style="width:100px;" /><a href="javascript:showCal('Calendar11')"><img align="top" src="img/date.png" border="0" /></a></td><td ><input type="hidden" name="link" value="31" /></td><td><input type="submit" value=" C a r i " class="text"/></td>
				</tr>
				</table>
				</form>
			</div>
			...
			<div id="table_search" style="min-height:300px;">
				<table width="95%" border="0" class="tb" cellspacing="0" cellspading="0">
					...
					while($data = mysql_fetch_array($qry)){
						$sql_1	= mysql_query('SELECT TGLBAYAR, SHIFT,TOTTARIFRS,SUM(JMBAYAR) AS total FROM t_bayarrajal WHERE TGLBAYAR ="'.$data['TGLBAYAR'].'" AND SHIFT ="1" AND STATUS = "LUNAS" and CARABAYAR = 1');
						$shift_1= mysql_fetch_array($sql_1);
						
						$sql_2	= mysql_query('SELECT TGLBAYAR, SHIFT,TOTTARIFRS,SUM(JMBAYAR) AS total FROM t_bayarrajal WHERE TGLBAYAR ="'.$data['TGLBAYAR'].'" AND SHIFT ="2" AND STATUS = "LUNAS" and CARABAYAR = 1');
						$shift_2= mysql_fetch_array($sql_2);
						
						$sql_3	= mysql_query('SELECT TGLBAYAR, SHIFT,TOTTARIFRS,SUM(JMBAYAR) AS total FROM t_bayarrajal WHERE TGLBAYAR ="'.$data['TGLBAYAR'].'" AND SHIFT ="3" AND STATUS = "LUNAS" and CARABAYAR = 1');
