...
#	$db = opendatabase();
	...
	if(!empty($_GET['grp'])){
		$myquery = "SELECT DISTINCT(nama_barang),m_barang.kode_barang FROM m_barang ".$join_unit."
		...
		if (mysql_num_rows ($userquery) > 0){
			...
			<?php
				while ($userdata = mysql_fetch_array ($userquery)){
					?><div onmouseover="this.style.background = '#EEEEEE'" onmouseout="this.style.background = '#CCCCCC'" onclick="setvalue_rad('<?php echo $userdata['nama_barang'];?>','<?php echo $userdata['kode_barang']; ?>')"><?php echo $userdata['nama_barang'];?></div><?php
				}
			?>
			</div>
			<?php
