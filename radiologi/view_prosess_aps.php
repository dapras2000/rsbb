...
<fieldset class="fieldset"><legend>Identitas Pasien</legend>
    <table width="100%" border="0" cellpadding="0" cellspacing="0" title=" From Ini Berfungsi Sebagai Form Entry Data Pasien Baru.">
        <tr>
            ...
            <td width="56%"><input title="NAMA" class="text" type="text" <?php if($_SESSION['NAMA']) {
    echo"value='".$_SESSION['NAMA']."'";
} ?> name="NAMA" size="30" value="<?=$m_pasien->NAMA?>" id="AWAL" /></td>
            <td width="24%" rowspan="11" valign="top"> Jenis Kelamin :<br />
                <input type="radio" name="JENISKELAMIN" value="L" <? if($m_pasien->JENISKELAMIN=="L" || $m_pasien->JENISKELAMIN=="l" || $_SESSION['JENISKELAMIN'] =="L")echo "Checked";?>/>
                Laki-laki<br />
                <input type="radio" name="JENISKELAMIN" value="P" <? if($m_pasien->JENISKELAMIN=="P" || $_SESSION['JENISKELAMIN'] =="P")echo "Checked";?>/>
                Perempuan<br />
                <br />
        ...
        <tr>
            ...
            <td>Tempat
                <input type="text" value="<? if($m_pasien->TEMPAT || $_SESSION['TEMPAT']) {
    echo $_SESSION['TEMPAT'].$m_pasien->TEMPAT;
}?>" class="text" name="TEMPAT" size="15" />
                <input onblur="calage(this.value,'umur');" type="text" class="text" value="<? if($m_pasien->TGLLAHIR || $_SESSION['TGLLAHIR']) {
    echo $_SESSION['TGLLAHIR'].$m_pasien->TGLLAHIR;
                }?>" name="TGLLAHIR" id="TGLLAHIR" size="20" />
...
?>
        ...
        <tr>
            ...
            <td colspan="1"><input name="ALAMAT" class="text" type="text" value="<? if($m_pasien->ALAMAT || $_SESSION['ALAMAT']) {
    echo $_SESSION['ALAMAT'].$m_pasien->ALAMAT;
} ?>" size="45" /></td>
        </tr>
        <tr>
          <td>Provinsi</td>
        ...
        <tr>
            ...
            <td><input  class="text" value="<? if($m_pasien->NOTELP || $_SESSION['NOTELP']) {
    echo $_SESSION['NOTELP'].$m_pasien->NOTELP;
}?>" type="text" name="NOTELP" size="25" /></td>
        </tr>
        <tr>
            <td>No KTP </td>
            <td><input  class="text" value="<? if($m_pasien->NOKTP || $_SESSION['NOKTP']) {
    echo $_SESSION['NOKTP'].$m_pasien->NOKTP;
}?>" type="text" name="NOKTP" size="25" /></td>
        </tr>
        <tr>
            <td>Nama Suami / Orang Tua Pasien</td>
            <td><input class="text" type="text" value="<? if($m_pasien->SUAMI_ORTU || $_SESSION['SUAMI_ORTU']) {
    echo $_SESSION['SUAMI_ORTU'].$m_pasien->SUAMI_ORTU;
}?>" name="SUAMI_ORTU" id="SUAMI_ORTU" size="25" /></td>
        </tr>
        <tr valign="top">
            <td valign="top">Pekerjaan Pasien / Orang Tua</td>
            <td><input class="text" type="text" value="<? if($m_pasien->PEKERJAAN || $_SESSION['PEKERJAAN']) {
    echo $_SESSION['PEKERJAAN'].$m_pasien->SUAMI_ORTU;
}?>" name="PEKERJAAN" size="25" /></td>
        </tr>
        <tr>
            <td valign="top">&nbsp;</td>
        ...
        <tr>
            <td colspan="3" align="right"><input type="submit" name="daftar" class="text" value="  S a v e  "/></td>
        </tr>
    </table>
