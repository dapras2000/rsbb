...
<div align="center">
    <div id="frame">
    <div id="frame_title">
      <h3>TAGIHAN FARMASI RAWAT INAP</h3></div>
      ...
      <input type="hidden" name="link" value="33depo_ranap" />
...
$kondisi=$knama.$knomr.$kpoly.$kbyr.$waktu;
	...
	$sql="SELECT a.NOMR, TRIM(c.NAMA) AS NAMA, d.nama AS POLY, e.NAMA AS CARABAYAR, b.STATUS, a.NOBILL,a.IDXDAFTAR,a.TANGGAL
FROM t_billranap a
JOIN t_bayarranap b ON a.NOBILL = b.NOBILL
JOIN m_pasien c ON a.NOMR = c.NOMR
JOIN m_ruang d ON d.no = a.KDPOLY
JOIN m_carabayar e ON a.CARABAYAR = e.KODE
WHERE a.KODETARIF LIKE '07%' ".$kondisi."
GROUP BY a.NOBILL";
	...
	$NO=0;
	$pager = new PS_Pagebill($connect, $sql, 15, 5, "start=".$_GET['start']."&end=".$_GET['end']."&poly=".$_GET['poly']."&cnama=".$_GET['cnama']."&cnomr=".$_GET['cnomr']."&crbayar=".$_GET['crbayar'],"index.php?link=33depo_ranap&");
	$rs = $pager->paginate();
	...
	while($data = mysql_fetch_array($rs)) {
				...
				$ssql	= mysql_query('select COUNT(*) as tagihan from t_bayarranap where NOMR = "'.$data['NOMR'].'" and IDXDAFTAR = "'.$data['IDXDAFTAR'].'" AND LUNAS = 0');
        ...
        ?>
              ...
              <td><a href="index.php?link=34depo_ranap&nomr=<?php echo $data['NOMR']; ?>&idxdaftar=<?php echo $data['IDXDAFTAR'];?>&nobill=<?php echo $data['NOBILL'];?>"> <input type="button" class="text" value="Prosess" /></a> <!--<a href="index.php?link=33batal&idxb=<? echo $data['NOBILL'];?>&idxdaftar=<? echo $data['IDXDAFTAR'];?>"><input type="button" class="text" value="Batal" /></a>--></td>
            </tr>
            <?	} 
