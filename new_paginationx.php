...
class PS_Pagination {
	...
	function PS_Pagination($connection, $sql, $sqlcounter,  $rows_per_page = 10, $links_per_page = 5, $append, $current_page) {
	...
	function paginate() {
		...
		$all_rs = @mysql_query($this->sqlcounter);
		if(!$all_rs) {
			if($this->debug) echo "SQL query failed. Check your query.<br /><br />Error Returned: ".mysql_error();
			return false;
		}
		while($data = mysql_fetch_row($all_rs))
    {
       $this->total_rows = $data[0];
    }
