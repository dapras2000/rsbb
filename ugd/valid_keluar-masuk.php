...
if($_POST['Masuk']){
   ...
   $_POST['IDXDAFTAR2'];
	...
	$qry_masuk 		= mysql_query("UPDATE t_pendaftaran 
							 		SET MASUKPOLY ='".$_POST['Masuk']."', 
							 		KDDOKTER = '".$_POST['dokter']."'
							 		WHERE NOMR = '".$_POST['NOMR']."' AND IDXDAFTAR='".$_POST['IDXDAFTAR2']."'")or die(mysql_error());
   ...
   $cek_keluar = 1;
	...
	if($status == "6"){			
   	...
   	if($cek_keluar == 1){ 
		...
		#$qry_keluar = mysql_query("UPDATE t_pendaftaran SET KELUARPOLY =CURTIME(), STATUS = '".$_POST['Status']."' WHERE NOMR = '".$_POST['NOMR']."' AND IDXDAFTAR='".$_POST['IDXDAFTAR2']."'")or die(mysql_error());
			if($qry_keluar){
				$keluar = $_POST['Keluar'];
				if($keluar == ''){
    				echo "<strong>Pasien Sudah dikeluarkan</strong>";
				}
				...
				if($status == "6"){ # RUJUK KE RUMAHSAKIT LAIN
				...
				if($status == "5"){ # RUJUK KE Poliklinik LAIN
	   	   			$kdpoly = $_POST['poly'];
				   	$kddokter = $_POST['dokter'];
					if(!empty($_POST['dokter'])){
						$kddokter = $_POST['dokter'];
					}
		    		$sqlsearchpendaftaran = "select NOMR, TGLREG, KDPOLY from t_pendaftaran WHERE NOMR = '".$nomr."' AND TGLREG = curdate() AND KDPOLY = '".trim($_POST['poly'])."'";
					...
					if(mysql_num_rows($rowpendaftaran) > 0){
						...
						/*
		   				$sqlinsert_pendaftaran = "INSERT INTO t_pendaftaran (NOMR, TGLREG, KDDOKTER, KDPOLY, KDRUJUK, KDCARABAYAR, NOJAMINAN, SHIFT, `STATUS`,
								  PASIENBARU, NIP) 
						VALUES('".$nomr."',curdate(),".$kddokter.",".$kdpoly.",".$kdrujuk.",".$kdcarabayar.",'".$nojaminan."',".$shift.",0,0,'".$nip."')";
		   				...
		   				*/
		   				$qrl_rujuk = "INSERT INTO t_alasan_rujuk (idxdaftar, status_rujuk, poly) 
						VALUE (".$_POST['IDXDAFTAR2'].",".$status.",".$_POST['poly'].")";
		   				mysql_query($qrl_rujuk); 
					}
				}
   			}
