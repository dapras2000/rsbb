...
?>
       <tr>
         ...
         <td><?=$data['QTY']?></td>
         <td><?=$data['satuan']?></td>
           <td><a href="#" onclick="javascript: MyAjaxRequest('validbarang','operasi/orderbarang/addbarangpengeluaran.php?optbarang=<?=$data['IDX']?>'); return false;" >Hapus</a></td>
      </tr>
<?php $i++; } ?>      
   </table>

   <form name="savebarang" id="savebarang" action="operasi/orderbarang/savebarangpengeluaran.php" method="post" >
   <input type="hidden" name="idxorder" value="<?=$_POST['idxorder']?>" />
   <input class="text" type="submit" value="S i m p a n" onclick="submitform (document.getElementById('savebarang'),'operasi/orderbarang/savebarangpengeluaran.php','validbarang',validatetask); return false;"/>
   </form>
   </fieldset>   
 <? } ?>
