	...
	jQuery('#NOMR').blur(function(){
		...
		if(nomr != ''){
			...
			jQuery.get('<?php echo _BASE_; ?>include/process.php?psn='+nomr,function(data){
				...
				jQuery('#ALAMAT_KTP').val(newdata[18]);
...
<div align="center">
  <div id="frame">
  <div id="frame_title"><h3 align="left">PENDAFTARAN PASIEN MELALUI TELEPON</h3>
</div>

<form name="myform" id="myform" action="models/daftar.php" method="post">
...
<table width="100%" border="0" style="background:none;" title=" From Ini Berfungsi Sebagai Form Pendaftaran Baru.">
      ...
      <tr id="tr_nomr">
        <td width="16%">No MR <?php #echo nomr("1");?></td>
      ...
      <tr>
        ...
        <td><input type="text" name="TGLREG" class="text" value="<?php if(!empty($_GET['TGLREG'])){ echo $_GET['TGLREG']; }else{ echo date("Y-m-d | H:i:s"); } ?>" size="25"/>
	  ...
	  while($ds = mysql_fetch_array($ss)){
		...
		echo '<input type="radio" name="KDCARABAYAR" id="'.$ds['KODE'].'" title="*" class="" '.$sel.' value="'.$ds['KODE'].'" /> '.$ds['NAMA'].'&nbsp;';
	  }
        ...
        <td colspan="2">
         <?php
		  ...
		  while($ds = mysql_fetch_array($ss)){
			...
			echo '<input type="radio" name="KDRUJUK" id="'.$ds['KODE'].'" title="*" class="" '.$sel.' value="'.$ds['KODE'].'" /> '.$ds['NAMA'].'&nbsp;';
		  }	
