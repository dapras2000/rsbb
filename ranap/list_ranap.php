...
$tgl_pesan="";
if($tgl_pesan !=""){
	$search = " WHERE DATE(a.masukrs) = '".$tgl_pesan."' ";
}else{
	$search = "";
}
...
if($ruang !="-Pilih Ruang-"){
	$search = $search." WHERE a.noruang ='".$ruang."' ";
	}
}
...
if($norm !=""){
	$search = $search." WHERE b.nomr = '".$norm."' ";
}
...
if($nama !=""){
	$search = $search." WHERE b.nama LIKE '%".$nama."%' ";
}
?>
...
<div align="center">
    <div id="frame">
    <div id="frame_title">
      <h3>LIST KUNJUNGAN RAWAT INAP</h3></div>
...
".$search." ORDER BY a.id_admission DESC";

	$pager = new PS_Pagination($connect, $sql, 15, 5, "tgl_kunjungan=".$tgl_kunjungan."&ruang=".$ruang."&nama=".$nama."&nomr=".$nomr,"index.php?link=12&");
	...
	while($data = mysql_fetch_array($rs)) {
      ...
      <td><?php if ($metu){echo "Sudah Pulang";}else{ echo "Belum Pulang";}
      	   ?>
      </td>
      <td>
      <?php if($data['keluarrs'] == ''){?>
