...
$tgl_pesan="";
if($tgl_pesan !=""){
	$search = " DATE(a.masukrs) = '".$tgl_pesan."' ";
}else{
	$search = "";
}
...
if($nama !=""){
	$search = $search." b.nama LIKE '%".$nama."%' ";
}
?>
...
<div align="center">
    <div id="frame">
    <div id="frame_title">
      <h3>LIST KUNJUNGAN RAWAT INAP NON AKTIF</h3></div>
...
?>"/><a href="javascript:showCal('Calendar3')"><img align="top" src="img/date.png" border="0" /></a>--></td>
       ...
       <td>
        ...
        <tr><td colspan="4" >      
              <input type="hidden" name="link" value="12nonaktif" />
  ...
  <tr>
    ...
    <th>Aksi</th>
          ...
          <?
$sql = "SELECT a.id_admission, a.keluarrs, b.nama as namapasien, a.tgl_pindah, a.nomr, b.alamat, a.statusbayar, c.nama as jenisbayar, a.masukrs, a.noruang, e.nama, a.nott, a.icd_masuk, a.NIP, d.jenis_penyakit 
FROM t_admission a
...
//echo $sql;
	$pager = new PS_Pagination($connect, $sql, 15, 5, "tgl_kunjungan=".$tgl_kunjungan."&ruang=".$ruang."&nama=".$nama."&nomr=".$nomr,"index.php?link=12nonaktif&");
	...
	while($data = mysql_fetch_array($rs)) {
      ...
      <td><?php echo "Sudah Pulang";?>
      </td>
      <td>
     
      			<a href='index.php?link=129z&indeks=<?php echo $data['id_admission']?>&no_ruang=<?php echo $data['noruang']?>&nott=<?php echo $data['nott']?>' ><input type='button' class='text' value='Batal Pulang' /></a>
