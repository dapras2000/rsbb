...
if(isset($_POST['nama_obat'])){
	if($_POST['jenis_obat']=="" || $_POST['kode_obat']=="" || $_POST['nama_obat']=="" || $_POST['waktu']=="" || $_POST['keterangan']==""){
		...
		if(isset($_POST['idxberiobat'])){
			mysql_query("UPDATE t_pemberianobat set IDXRANAP='$_POST[id_admission]', NOMR='$_POST[nomr]', TANGGAL='$_POST[tanggal]', DOKTER='$_POST[kddokter]', JENIS='$_POST[jenis_obat]', KODE_OBAT='$_POST[kode_obat]', NAMA='$_POST[nama_obat]', WAKTU='$_POST[waktu]', KETERANGAN='$_POST[keterangan]' where IDXBERIOBAT='$_POST[idxberiobat]';")or die(mysql_error());
			echo "<div style='border:1px solid #DF7; width:95%; padding:5px; margin:5px; color:#F30; background-color:#FFF;' align='left'>";
			echo "<div style='color:#090;'><strong>Edit Data Sukses!</strong></div>";
			echo "</div>";	
		}else{
			mysql_query("INSERT INTO t_pemberianobat VALUES('','$_POST[id_admission]','$_POST[nomr]',now(),'$_POST[kddokter]','$_POST[jenis_obat]','$_POST[kode_obat]','$_POST[nama_obat]','$_POST[waktu]','$_POST[keterangan]','')")or die(mysql_error());
			echo "<div style='border:1px solid #DF7; width:95%; padding:5px; margin:5px; color:#F30; background-color:#FFF;' align='left'>";
...
?>
	...
	<td><input type="submit" size="50" name="edit" value="Edit" class="text" onclick="newsubmitform (document.getElementById('ranap'),'ranap/form_ranap.php?edit=ok&idxberiobat=<?=$data['IDXBERIOBAT']?>&id_admission=<?=$_POST['id_admission']?>&nomr=<?=$_POST['nomr']?>&kddokter=<?=$_POST['kddokter']?>&jenis=<?=$data['JENIS']?>&kodeobat=<?=$data['KODE_OBAT']; ?>&nama=<?=$data['NAMA']?>&tanggal=<?=$data['TANGGAL']?>&waktu=<?=$data['WAKTU']?>&keterangan=<?=$data['KETERANGAN']?>','loadform',validatetask); return false;"/></td>
  </tr>
<?php } ?>
</table>
</div>
