...
$sql_rad3 = "SELECT DISTINCT m_radiologi.kd_rad, m_radiologi.gr_rad, m_radiologi.nama_rad
			FROM m_radiologi
			WHERE m_radiologi.gr_rad = '-' AND m_radiologi.tab_view = 3 AND paket = '2'";
$get_rad3 = mysql_query($sql_rad3);

?>
<form name="rontagen" id="rontagen" action="ranap/rad/valid_rontgen.php" method="post">
<table width="60%" >
<tr><td valign="top">
...
?>
  <tr>
    ...
    <td width="167" colspan="3">
    <? 
	   $sql_cek3 = "SELECT IDXORDERRAD FROM t_radiologi WHERE JENISPHOTO = '".$dat_rads3['kd_rad']."'
	   				AND IDXDAFTAR = ".$idxdaftar;
	   $get_cek3 = mysql_query($sql_cek3);
	   $dat_cek3 = mysql_fetch_assoc($get_cek3);
	   $cek3 = $dat_cek3['IDXORDERRAD'];
	?>
    <input type="checkbox" name="rad<?=$dat_rads3['kd_rad']?>" value="<?=$dat_rads3['kd_rad']?>" 
    <? if($cek3 != "") echo "checked=checked"; ?>/>&nbsp;<?=$dat_rads3['nama_rad']?></td>
  </tr>
<? } 
}
?>  
...
<input type="submit" name="saved" class="text" value=" Simpan " onclick="newsubmitform (document.getElementById('rontagen'),'ranap/rad/valid_rontgen.php','val_rontagen',validatetask); return false;"/>
</td>
</tr>
</table>
</div>
