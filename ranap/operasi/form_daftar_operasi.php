...
<? 
$sql_operasi = "select * from t_operasi where idxdaftar = ".$_POST['id_admission']." and status != 'selesai' ";
$get_operasi = mysql_query($sql_operasi);
$dat_operasi = mysql_fetch_assoc($get_operasi);
?>
<form action="ranap/operasi/tambah_daftar_operasi.php" name="daftar" id="daftar" method="post">
<table width="95%" class="tb" border="0" cellpadding="0" cellspacing="0">
  ...
  <tr>
    ...
    <td>&nbsp;</td>
    <td><label>
      <input type="hidden" name="idxdaftar" value="<?=$_POST['id_admission']?>" /> 	
      <input type="hidden" name="nomr" value="<?=$_POST['nomr']?>" /> 
      <input type="hidden" name="kdpoly" value="<?=$_POST['noruang']?>" /> 	
      <input type="hidden" name="kddokter" value="<?=$_POST['kddokter']?>" /> 	
