    ...
    echo $nama;
            ...
            <div id="table_search">
                <table class="tb" width="95%" style="margin:10px;" border="0" cellspacing="1" cellspading="1" title="List Kunjungan Data Pasien Per Hari Ini">
                    ...
                    $NO		= 0;
					/*
					$sql	= 	'SELECT DISTINCT a.nomr,a.idxdaftar,a.tanggal,a.KDPOLY, a.DRPENGIRIM, a.RAJAL, a.APS, e.NAMA AS carabayar,
b.NAMADOKTER, c.NAMA, c.ALAMAT
FROM t_orderlab a
LEFT JOIN m_dokter b ON b.KDDOKTER = a.DRPENGIRIM 
JOIN m_pasien c ON c.NOMR = a.NOMR
JOIN t_pendaftaran d ON d.IDXDAFTAR = a.IDXDAFTAR
JOIN m_carabayar e ON e.KODE = d.KDCARABAYAR
JOIN m_dokter f 
									WHERE a.STATUS = 0 and a.APS = 0 '.$search; 
									*/
$sql = 'SELECT DISTINCT a.nomr,a.idxdaftar,a.tanggal,a.KDPOLY, a.DRPENGIRIM, a.RAJAL, a.NOLAB,
...
CASE aps WHEN 1 THEN (SELECT alamat FROM m_pasien_aps b WHERE b.NOMR=a.NOMR) 
ELSE (SELECT alamat FROM m_pasien b WHERE b.NOMR=a.NOMR) END AS alamat,
CASE rajal WHEN 1 THEN (SELECT m_unit.nama_unit FROM m_unit WHERE m_unit.kode_unit = a.KDPOLY) 
ELSE (SELECT m_ruang.nama FROM m_ruang WHERE m_ruang.no = a.KDPOLY) END AS poly_kelas,
m_dokter.NAMADOKTER, t_pendaftaran.KDCARABAYAR, m_carabayar.NAMA AS carabayar
FROM t_orderlab a
...
INNER JOIN m_pasien b ON a.nomr = b.nomr AND b.nama LIKE "%'.$nama.'%"
WHERE a.STATUS = 0 '.$search.'
UNION
SELECT DISTINCT a.nomr,a.idxdaftar,a.tanggal,a.KDPOLY, a.DRPENGIRIM, a.RAJAL, a.NOLAB,
CASE aps WHEN 1 THEN (SELECT nama FROM m_pasien_aps b WHERE b.NOMR=a.NOMR ) 
ELSE (SELECT nama FROM m_pasien b WHERE b.NOMR=a.NOMR) END AS nama,
CASE aps WHEN 1 THEN (SELECT alamat FROM m_pasien_aps b WHERE b.NOMR=a.NOMR) 
ELSE (SELECT alamat FROM m_pasien b WHERE b.NOMR=a.NOMR) END AS alamat,
...
//The paginate() function returns a mysql result set 
    ...
    ($hal*15)+$NO;?></td>
                        ...
                        <td><a href="index.php?link=62&amp;nomr=<?php echo $data['nomr']?>&idx=<?php echo $data['idxdaftar']?>&rajal=<?php echo $data['RAJAL']?>&drkirim=<? echo $data['DRPENGIRIM'];?>&nolab=<? echo $data['NOLAB'];?>" ><input type="button" value="PERIKSA" class="text"/></a></td>
                    </tr>
                        <?	}
