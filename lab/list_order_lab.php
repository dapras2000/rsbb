    ...
    echo $nama;
                    ...
                    <tr>
                        ...
                        <td><input type="submit" value="Cari" class="text"/>
                            <input type="hidden" name="link" value="6" /></td>
                    </tr>
                </table>
            ...
            <div id="table_search">
				...
				<tr align="center"><th>No</th><th>No RM</th><th>Tanggal</th><th>Nama Pasien</th><th>Alamat</th><th>Poli/ Ruang</th><th>Dokter Pengirim</th><th width="70px">Cara Bayar</th><th width="200px">&nbsp;</th></tr>
                    <?
                    $NO=0;
                    $sql = "(SELECT a.UNIT, a.NOMR, a.IDXDAFTAR, a.TGLDAFTAR, a.NIP, a.IDXORDERLAB, b.NAMA, b.ALAMAT, d.NAMA AS CARABAYAR,c.nama_unit, e.NAMADOKTER 
FROM tmp_cartorderlab a 
JOIN m_pasien b ON a.NOMR = b.NOMR 
JOIN m_unit c ON a.UNIT = c.kode_unit 
JOIN t_pendaftaran f ON f.IDXDAFTAR = a.IDXDAFTAR
LEFT JOIN m_carabayar d ON f.KDCARABAYAR= d.KODE 
LEFT JOIN m_dokter e ON e.KDDOKTER = a.KDDOKTER 
".$search." GROUP BY a.NOMR, a.TGLDAFTAR) 
					UNION
(SELECT a.UNIT, a.NOMR, a.IDXDAFTAR, a.TGLDAFTAR, a.NIP, a.IDXORDERLAB, b.NAMA, b.ALAMAT, d.NAMA AS CARABAYAR,c.nama_unit, e.NAMADOKTER
FROM tmp_cartorderlab a 
JOIN m_pasien_aps b ON a.NOMR = b.NOMR 
JOIN m_unit c ON a.UNIT = c.kode_unit 
JOIN t_pendaftaran_aps f ON f.IDXDAFTAR = a.IDXDAFTAR
LEFT JOIN m_carabayar d ON f.KDCARABAYAR= d.KODE 
LEFT JOIN m_dokter e ON e.KDDOKTER = a.KDDOKTER ".$search."  group by a.NOMR, a.TGLDAFTAR) order by NAMA asc";
                    ...
                    <?php
                        ...
                        <td>
						<?php
							echo '<a href="'._BASE_.'index.php?link=62formorderlab&nomr='.$data['NOMR'].'&idx='.$data['IDXDAFTAR'].'"><input type="button" value="BATAL" class="text"/></a> <a href="'._BASE_.'index.php?link=62formorderlab&nomr='.$data['NOMR'].'&idx='.$data['IDXDAFTAR'].'"><input type="button" value="TAMBAH PEMERIKSAAN" class="text"/></a>';
						?>
                        </td>
                    </tr>
                        <?	$i++;
