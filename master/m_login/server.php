...
function createGrid($start = 0, $limit = 1, $filter = null, $content = null, $order = null, $divName = "grid", $ordering = ""){
	...
	$eventHeader = array();
	$eventHeader[]= 'onClick=\'xajax_showGrid(0,'.$limit.',"'.$filter.'","'.$content.'","nip","'.$divName.'","ORDERING");return false;\'';
	$eventHeader[]= 'onClick=\'xajax_showGrid(0,'.$limit.',"'.$filter.'","'.$content.'","pwd","'.$divName.'","ORDERING");return false;\'';
	$eventHeader[]= 'onClick=\'xajax_showGrid(0,'.$limit.',"'.$filter.'","'.$content.'","ses_reg","'.$divName.'","ORDERING");return false;\'';
	
	$eventHeader[]= 'onClick=\'xajax_showGrid(0,'.$limit.',"'.$filter.'","'.$content.'","roles","'.$divName.'","ORDERING");return false;\'';
	//$eventHeader[]= 'onClick=\'xajax_showGrid(0,'.$limit.',"'.$filter.'","'.$content.'","kdunit","'.$divName.'","ORDERING");return false;\'';
	$eventHeader[]= 'onClick=\'xajax_showGrid(0,'.$limit.',"'.$filter.'","'.$content.'","departemen","'.$divName.'","ORDERING");return false;\'';

	// Select Box: fields table.
	$fieldsFromSearch = array();
	$fieldsFromSearch[] = 'nip';
	$fieldsFromSearch[] = 'roles';
	//$fieldsFromSearch[] = 'kdunit';
	$fieldsFromSearch[] = 'departemen';

	// Selecct Box: Labels showed on search select box.
	$fieldsFromSearchShowAs = array();
	$fieldsFromSearchShowAs[] = "NIP ";
	$fieldsFromSearchShowAs[] = "ROLES";
	//$fieldsFromSearchShowAs[] = "KDUNIT";
	$fieldsFromSearchShowAs[] = "DEPARTEMEN";
	// Create object whit 5 cols and all data arrays set before.
	$table = new ScrollTable(6,$start,$limit,$filter,$numRows,$content,$order);
	$table->setHeader('title',$headers,$attribsHeader,$eventHeader);
	$table->setAttribsCols($attribsCols);
	$table->addRowSearch("m_login,",$fieldsFromSearch,$fieldsFromSearchShowAs);
	...
	// Change here by the name of fields of its database table
		...
		//$rowc[] = $row['kdunit'];
		$rowc[] = $row['departemen'];		
		$table->addRow("m_login",$rowc,1,1,$divName,$fields);
 	}
