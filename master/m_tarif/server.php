...
function createGrid($start = 0, $limit = 1, $filter = null, $content = null, $order = null, $divName = "grid", $ordering = ""){
	...
	$fields = array();
	$fields[] = 'kode_lampiran';
	$fields[] = 'nama_gruptindakan';
	$fields[] = 'nama_tindakan';
	$fields[] = 'kelas';

	// HTML table: Headers showed
	$headers = array();
	$headers[] = "KODE LAMPIRAN ";
	$headers[] = "NAMA GRUP TINDAKAN";
	$headers[] = "NAMA TINDAKAN";
	$headers[] = "KELAS";
...
/*	$eventHeader = array();
	$eventHeader[]= 'onClick=\'xajax_showGrid(0,'.$limit.',"'.$filter.'","'.$content.'","kode","'.$divName.'","ORDERING");return false;\'';
	$eventHeader[]= 'onClick=\'xajax_showGrid(0,'.$limit.',"'.$filter.'","'.$content.'","nama","'.$divName.'","ORDERING");return false;\'';
	//$eventHeader[]= 'onClick=\'xajax_showGrid(0,'.$limit.',"'.$filter.'","'.$content.'","kdpoly","'.$divName.'","ORDERING");return false;\'';*/

	// Select Box: fields table.
	$fieldsFromSearch = array();
	$fieldsFromSearch[] = 'kode_lampiran';
	$fieldsFromSearch[] = 'nama_gruptindakan';
	$fieldsFromSearch[] = 'nama_tindakan';
	$fieldsFromSearch[] = 'kelas';

	// Selecct Box: Labels showed on search select box.
	$fieldsFromSearchShowAs = array();
	$fieldsFromSearchShowAs[] = "KODE LAMPIRAN";
	$fieldsFromSearchShowAs[] = "NAMA GRUP TINDAKAN";	
	$fieldsFromSearchShowAs[] = "NAMA TINDAKAN";	
	$fieldsFromSearchShowAs[] = "KELAS";

	// Create object whit 5 cols and all data arrays set before.
	$table = new ScrollTable(6,$start,$limit,$filter,$numRows,$content,$order);
	$table->setHeader('title',$headers,$attribsHeader,$eventHeader);
	$table->setAttribsCols($attribsCols);
	$table->addRowSearch("m_tarif2012",$fieldsFromSearch,$fieldsFromSearchShowAs);
	
	while ($arreglo->fetchInto($row)) {
	// Change here by the name of fields of its database table
		$rowc = array();
		$rowc[] = $row['kode_lampiran'];
		$rowc[] = $row['kode_lampiran'];
		$rowc[] = $row['nama_gruptindakan'];
		$rowc[] = $row['nama_tindakan'];
		$rowc[] = $row['kelas'];
		$table->addRow("m_tarif2012",$rowc,1,1,$divName,$fields);
 	}
