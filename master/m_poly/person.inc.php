...
class Person extends PEAR
{
	...
	function &getAllRecords($start, $limit, $order = null){
		...
		if($order == null){
			$sql = "select kode,nama
					from m_poly LIMIT $start, $limit ".$_SESSION['ordering'];
		}else{
			$sql = "select kode,nama
	...
	*/
	...
	function &getRecordsFiltered($start, $limit, $filter = null, $content = null, $order = null, $ordering = ""){
		...
		if(($filter != null) and ($content != null)){
			  $sql = "select kode,nama
					from m_poly where ".$filter." like '%".$content."%' "
					." ORDER BY ".$order
					." ".$_SESSION['ordering']
	...
	*/
	...
	function &getNumRows($filter = null, $content = null){
		...
		$sql = "SELECT COUNT(*) AS numRows FROM m_poly";
		
		if(($filter != null) and ($content != null)){
			$sql = 	"select COUNT(*) AS numRows from m_poly where  ".$filter." like '%$content%'";
		}
		Person::events($sql);
		$res =& $db->getOne($sql);
		return $res;		
	...
	*/
	...
	function &getRecordByID($id){
		...
		$sql = "select kode,nama	from m_poly where kode = '$id'";
		Person::events($sql);
		$row =& $db->getRow($sql);
		return $row;
	}
	...
	*/
	...
	function deleteRecord($id){
		...
		$sql = "DELETE FROM m_poly WHERE kode = '$id'";
		Person::events($sql);
		$res =& $db->query($sql);
		return $res;
	}
	...
	*/
	...
	function formEdit($id){		
		...
		$html = '
				...
				<!-- No edit the next line -->
				<input type="hidden" id="id"  name="id" value="'.$person['kode'].'">

				<table border="0" width="100%">
				<tr>
					<td nowrap align="left">Kode *</td>
					<td align="left"><input type="text" id="v_kode" name="v_kode" size="25" readonly="true" value="'.$person['kode'].'"></td>
				</tr>
				<tr>
					<td nowrap align="left">Nama Poliklinik *</td>
					<td align="left"><input type="text" id="v_nama" name="v_nama" size="25" value="'.$person['nama'].'"></td>
				</tr>
				<tr>
	...
	*/
	function checkAllData($f,$new = 0){
		if(empty($f['v_kode'])) return "Kode harus diisi";
		if(empty($f['v_nama'])) return "Nama Poliklinik harus diisi";
