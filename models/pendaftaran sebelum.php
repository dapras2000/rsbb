...
<SCRIPT language="JavaScript">
    ...
    if($_POST['KDPOLY']=="9" || $_POST['KDPOLY']=="10") {
		#print_r($_SESSION);
        $sqlinsert_pendaftaran = "INSERT INTO t_pendaftaran (NOMR,TGLREG,KDDOKTER,KDPOLY,KDRUJUK,KDCARABAYAR,NOJAMINAN,JAMREG, MASUKPOLY,MINTA_RUJUKAN,SHIFT,PASIENBARU,NIP,KETRUJUK,PENANGGUNGJAWAB_NAMA, PENANGGUNGJAWAB_HUBUNGAN, PENANGGUNGJAWAB_ALAMAT, PENANGGUNGJAWAB_PHONE,status,KETBAYAR,NOKARTU,NORUJUKAN,TGLRUJUKAN,DIAGNOSA_AWAL) VALUES('".$nomr."','".trim($_POST['TGLREG'])."',".$dokter.",".trim($_POST['KDPOLY']).",".trim($_POST['KDRUJUK']).",".trim($_POST['KDCARABAYAR']).",'".trim($_POST['NOJAMINAN'])."', now(), current_time(), '".$minta_rujukan."',".trim($_POST['SHIFT']).",".$status.",'".$_SESSION['NIP']."','".trim($_POST['KETRUJUK'])."','".trim($_POST['nama_penanggungjawab'])."', '".trim($_POST['hubungan_penanggungjawab'])."', '".trim($_POST['alamat_penanggungjawab'])."', '".trim($_POST['phone_penanggungjawab'])."',0,'".$_REQUEST['KETBAYAR']."','".trim($_POST['NK'])."','".trim($_POST['norujukan'])."','".trim($_POST['tglrujuk'])."','".trim($_POST['diagnosa'])."')";
    }else{
        $sqlinsert_pendaftaran = "INSERT INTO t_pendaftaran (NOMR, TGLREG, KDDOKTER, KDPOLY, KDRUJUK, KDCARABAYAR, NOJAMINAN, SHIFT, STATUS, PASIENBARU, NIP, KETRUJUK, PENANGGUNGJAWAB_NAMA, PENANGGUNGJAWAB_HUBUNGAN, PENANGGUNGJAWAB_ALAMAT, PENANGGUNGJAWAB_PHONE, JAMREG, MINTA_RUJUKAN,KETBAYAR,NOKARTU,NORUJUKAN,TGLRUJUKAN,DIAGNOSA_AWAL)
								VALUES('".$nomr."','".trim($_POST['TGLREG'])."',".$dokter.",".trim($_POST['KDPOLY']).",".trim($_POST['KDRUJUK']).",".trim($_POST['KDCARABAYAR']).",'".trim($_POST['NOJAMINAN'])."',".trim($_POST['SHIFT']).",0,".$status.",'".trim($_SESSION['NIP'])."','".trim($_POST['KETRUJUK'])."', '".trim($_POST['nama_penanggungjawab'])."', '".trim($_POST['hubungan_penanggungjawab'])."', '".trim($_POST['alamat_penanggungjawab'])."', '".trim($_POST['phone_penanggungjawab'])."', now(), '".$minta_rujukan."','".$_REQUEST['KETBAYAR']."','".trim($_POST['NK'])."','".trim($_POST['norujukan'])."','".trim($_POST['tglrujuk'])."','".trim($_POST['diagnosa'])."')";						
    }
	...
	$sql='CALL pr_savebill_tindakanrajal_dokter("'.$nomr.'",'.$_REQUEST['SHIFT'].',"'.$_SESSION['NIP'].'","'.$last_idxdaftar.'",CURDATE(),0,0,"'.$ip.'",'.$_REQUEST['KDCARABAYAR'].','.$_REQUEST['KDPOLY'].',0,"'.$_REQUEST['KDDOKTER'].'","'.$_REQUEST['KDPOLY'].'")';
	mysql_query($sql);
	/*
...
<script>
function cetakkartu(){
window.open("../pdfb/kartupasien.php?NOMR=<?php echo $_SESSION['register_nomr']; ?>&NAMA=<?php echo $_SESSION['register_nama'] ?>&ALAMAT=<?php echo $_REQUEST['ALAMAT'];?>&JENISKELAMIN=<?php echo $_REQUEST['JENISKELAMIN'];?>&TGLLAHIR=<?php echo $_REQUEST['TGLLAHIR'];?>");
}
...
jQuery(document).ready(function(){
	jQuery('.printrm').click(function(){
		...
		jQuery.post('<?php echo _BASE_.'print_tracer.php';?>',{kode:<?php echo $_POST['PASIENBARU'];?>,idx:<?php echo $_SESSION['idx'];?>,poly:<?php echo $_SESSION['poly'];?>,status:<?php echo $_SESSION['status'];?>},function(data){
...
jQuery(document).ready(function(){
	jQuery('.printsep').click(function(){
		//jQuery('#show_print').printArea();
		//return false;
		//alert('asda');
		var idx 	= jQuery('#idx').val();
		var poly	= jQuery('#poly').val();
		jQuery.post('<?php echo _BASE_.'print_sep.php';?>',{idx:<?php echo $_SESSION['idx'];?>,poly:<?php echo $_SESSION['poly'];?>},function(data){
			jQuery('#show_print').empty().html(data);
			w=window.open();
			w.document.write(jQuery('#show_print').html());
			w.print();
...
<div style="background:#D7F5A1; width:auto; margin-left:auto; margin-right:auto; text-align:left; margin-top:auto;">
	<div style="font-size:14px; color : green ">
	  <p>Data Telah di Simpan.</p>
    </div>
    <div style="font-size:20px;">
      <p>NO RM : <?php echo $_SESSION['register_nomr']; ?></p>
    </div>
    <div style="font-size:20px;"></div>
    <div style="font-size:20px;">NAMA PASIEN : <?php echo $_SESSION['register_nama']; ?></div>
    <div style="font-size:20px;"></div>
    <p>
  ...
  &nbsp;&nbsp;
  <input type="button" name="print_sep" value="Cetak SEP" class="text print"  class="printsep"/>
      &nbsp;&nbsp;
	  <input type="button" name="back"  class="printrm" value="Cetak SKP" />
</p>
    <p>&nbsp;</p>
    <p>&nbsp;</p>
    <p>&nbsp;</p>
