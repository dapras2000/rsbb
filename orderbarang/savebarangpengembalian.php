...
$sql = "SELECT KDBARANG, QTY, NIP,
		...
		$row = mysql_query($sql)or die(mysql_error());
			if(count($row) > 0){
				  while($data = mysql_fetch_array($row)){
					  ...
					  @mysql_query("INSERT INTO t_pengembalian_barang (NIP, KDUNIT, kodebarang,
		...
		@mysql_query("DELETE FROM temp_cartbarang_pengembalian WHERE IP = '$ip'")or die(mysql_error());
		echo "<fieldset class='fieldset'>";
        echo "<legend>Informasi</legend>";
		echo "Simpan Data Pengembalian Barang Berhasil."; 
		echo "</legend>"; 
?>