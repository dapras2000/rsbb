...
<div align="center">
    <div id="frame">
        <div id="frame_title">
            <h3>LIST  PASIEN RAWAT JALAN</h3></div>
                ...
                <table width="248" border="0" cellspacing="0" class="tb">
                    ...
                    <tr>
                        ...
                        <td><input type="submit" value="Cari" class="text"/>
                            <input type="hidden" name="link" value="rm4" /></td>
                    </tr>
                </table>
		...
		FROM
  		m_pasien 
		INNER JOIN t_pendaftaran ON (m_pasien.NOMR=t_pendaftaran.NOMR)
		".$search;
		
		 $sqlcounter = "SELECT DISTINCT
  		count(m_pasien.NOMR)
		FROM
  		m_pasien
		INNER JOIN t_pendaftaran ON (m_pasien.NOMR=t_pendaftaran.NOMR)
		".$search;
                        ...
                        $sql = "SELECT a.NOMR, a.NAMA, a.ALAMAT, a.JENISKELAMIN, a.TGLLAHIR, b.KDPOLY FROM m_pasien a, t_pendaftaran b WHERE a.NOMR=b.NOMR AND b.IDXDAFTAR=".$_GET['idx']." AND b.KDPOLY=".$_SESSION['KDUNIT'];
                        $sqlcounter = "SELECT count(a.NOMR) FROM m_pasien a, t_pendaftaran b WHERE a.NOMR=b.NOMR AND b.IDXDAFTAR=".$_GET['idx']." AND b.KDPOLY=".$_SESSION['KDUNIT'];
 }

                                        $NO=0;
                    $pager = new PS_Pagination($connect, $sql, $sqlcounter, 15, 5, "nama=".$nama."&norm=".$norm,"index.php?link=rm4&");
