  ...
  <?php do { ?>
    <tr bgcolor="#FFF9DF">
      ...
      <td><div align="left">
		...
		$detail="SELECT idxruang,no_tt FROM m_detail_tempat_tidur WHERE idxruang='".$idx."' order by NO_TT asc";
	...
	while($brs=@mysql_fetch_array($result)){
		 ...
		 if ($jumlahseleksi>0){
			...
			echo "<input $gaya type=button name=button id=".$brs['no_tt']." value=".$brs['no_tt']." onclick=window.location.href='?link=17&idruang=".$brs['idxruang']."&p=daftar&no=".$_GET['no']."&no_tt=".$brs['no_tt']."&namaruang=".$namarg."';>";
		}
	}
	  ?>
      </div></td>
