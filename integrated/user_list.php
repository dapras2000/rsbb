...
<th><div align="center">TW IV</div></th>
  ...
  if ($find==""){
$n=1;
$sql= "select `KODE RS` as kode1, left(`KODE RS`,2) AS dinkes, left(`KODE RS`,1) AS puskesmas, `NAMA RS` as rs, kodeprop, kodeprop_eplan from kode where kodeprop='$_SESSION[kodeprop]' and left(`KODE RS`,1)<>'P' order by `KODE RS` ASC";
$hasil=mysql_query($sql);
}
else{
$n=1;
$sql= "select `KODE RS` as kode1, left(`KODE RS`,2) AS dinkes, left(`KODE RS`,1) AS puskesmas, `NAMA RS` as rs, kodeprop, kodeprop_eplan from kode where kodeprop='$kodeprop' and left(`KODE RS`,1)<>'P' and `NAMA RS` like '%$find%' order by `KODE RS` ASC";
$hasil=mysql_query($sql);
}
...
else if($r2[koders]<1 && $r1[dinkes]==$r1[kodeprop_eplan]){
$icon_rs="images/bullet-cog.png";}
else if($r2[koders]<1 && $r1[puskesmas]=="P"){
$icon_rs="images/bullet-cog.png";}
else{
...
$sql4="SELECT COUNT(kd_satker) as koders from user_eplanning where kd_satker='$r1[kode1]'";
...
$sql5="SELECT COUNT(kd_satker) as koders from user_edak where kd_satker='$r1[kode1]'";
...
$sql13="SELECT COUNT(koders) as koders from user_emonev where koders='$r1[kode1]'";
