...
jQuery(document).ready(function(){
	jQuery('#simpan_racikan').click(function(){
		...
		jQuery.post('<?php echo _BASE_;?>apotek/simpan_obat_tmp_cartresep.php',{idx:idx,kode:nama_racikan,sediaan:sediaan,aturan:aturan,qty:qty,total:total,dokter:dokter},function(data){
			jQuery('#validbarang').load('<?php echo _BASE_;?>apotek/load_obat_tmp_cartresep_rajal.php?idxdaftar='+idx);
	...
	jQuery('.batal_racik').click(function(){
		...
		jQuery.post('<?php echo _BASE_;?>apotek/delete_tmp_racikan_obat.php',{idobat:idobat},function(data){
			jQuery('#table_list_racikan').load('<?php echo _BASE_;?>apotek/load_obat_racikan_tmpracikan_rajal.php?txtIdxDaftar='+idx);
		});
	});
});
</script>
...
/*$sql = 'SELECT a.*, b.nama_obat
				   ...
				   */
$sql='SELECT a.*, b.nama_barang FROM tmp_racikan_obat a JOIN m_barang b ON b.kode_barang = a.KODE_OBAT where a.IDXDAFTAR= '.$_REQUEST['txtIdxDaftar'];
$sql = mysql_query($sql);
if(mysql_num_rows($sql) > 0){
	...
	while($data = mysql_fetch_array($sql)){
		echo '<tr><td>'.$i.'</td><td>'.$data['nama_barang'].'</td><td>'.$data['JUMLAH'].'</td><td align="right">'.curformat($data['HARGA_OBAT'],2).'</td><td align="right">'.curformat($data['JUMLAH'] * ($data['HARGA_OBAT']),2).'</td><td><input value="Batal"  type="button" idx="'.$_REQUEST['txtIdxDaftar'].'" id="'.$data['IDXRACIK'].'" class="batal_racik text"></td><tr>';
		$i++;
