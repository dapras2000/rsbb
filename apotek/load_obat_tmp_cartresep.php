...
if($_REQUEST['simpan_obat'] == 'Simpan'){
	//message_back($_SESSION['apt_type']);
	$ip	= getRealIpAddr();
	...
	//danang
			//$sqlrsp = "select max(substr(noresep,7)) as nrsp from t_billobat_rajal where idxdaftar ='$_REQUEST[idxdaftar]'";
			$sqlrsp = "select max(noresep) as nrsp from t_billobat_rajal where idxdaftar ='$_REQUEST[idxdaftar]'";
			$qryrsp = mysql_query($sqlrsp);
			$hslrsp = mysql_fetch_array($qryrsp);
			//$_SESSION['norsp'] = $hslrsp['nrsp'];
			$noresepe = $hslrsp['nrsp'];

			$sqlrsp2 = "select max(noresep) as nrsp from t_billobat_ranap where idxdaftar ='$_REQUEST[idxdaftar]'";
			$qryrsp2 = mysql_query($sqlrsp2);
			$hslrsp2 = mysql_fetch_array($qryrsp2);
			//$_SESSION['norsp'] = $hslrsp['nrsp'];
			$noresepe2 = $hslrsp2['nrsp'];

	if ($_SESSION['apotekrajal'] == '0'){
		?>
	    <SCRIPT language="JavaScript">
			alert("Data Telah Disimpan.");
			window.location="../index.php?link=add_resep_ranap<?php echo $aps_status;?>&nomr=<?php echo $row['NOMR'];?>&idx=<?php echo $_REQUEST['idxdaftar']?>&rajal=<?php echo $_SESSION['apotekrajal'];?>&noresep=<?php echo $noresepe2;?>";
		</SCRIPT>		
		<?
	}else{
		?>
	    <SCRIPT language="JavaScript">
			alert("Data Telah Disimpan.");
			window.location="../index.php?link=add_resep<?php echo $aps_status;?>&nomr=<?php echo $row['NOMR'];?>&idx=<?php echo $_REQUEST['idxdaftar']?>&rajal=<?php echo $_SESSION['apotekrajal'];?>&noresep=<?php echo $noresepe;?>";
		</SCRIPT>
		<?
	}
}

$sql = "SELECT c.*, o.nama_barang
FROM tmp_cartresep c
JOIN m_barang o ON o.kode_barang = c.kode_obat
