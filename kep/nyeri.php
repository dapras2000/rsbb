...
<script>
    $(document).ready(function() {
        $("#tgl_nyeri").datepicker({dateFormat: 'dd-mm-yy'});
        $("#formNyeri").validate({
            debug: false,
            messages: {
                tgl: "wajib diisi !",
            ...
            submitHandler: function(form) {
                ...
                $.post('kep/nyeri_saved.php', $("#formNyeri").serialize(), function(data) {
                    alert('data berhasil disimpan');
                    $('#formNyeri').each(function() {
                        this.reset();
                    });
                    $('#table_nyeri').load('kep/nyeri_table.php');
//                    $('#results').html(data);
                });
            }
        });
...
<div id="result" ></div>
<fieldset><legend>Input Data Nyeri</legend>

    <form id="formNyeri" method="post" action="">
        <table width="100%" border="0" cellpadding="4" cellspacing="2">

            <tr>
                <td width="200"><label for="tgl" id="tgl_nyeri_label">Tgl</label></td>
                <td><input type="text" name="tgl" class="required" id="tgl_nyeri"> <b>(format:dd-mm-YYYY)</b></td>
            </tr>
