...
<script>
    $(document).ready(function() {
        $("#tgl_kenyamanan").datepicker({dateFormat: 'dd-mm-yy'});
        $("#formKenyamanan").validate({
            debug: false,
            messages: {
                tgl: "wajib diisi !",
                jml: "wajib diisi !",
            ...
            submitHandler: function(form) {
                ...
                $.post('kep/kenyamanan_saved.php', $("#formKenyamanan").serialize(), function(data) {
                    alert('data berhasil disimpan');
                    $('#formKenyamanan').each(function() {
                        this.reset();
                    });
                    $('#table_kenyamanan').load('kep/kenyamanan_table.php');
//                    $('#results').html(data);
                });
            }
        });
...
//            return true;
            $.ajax({
                url: 'kep/kenyamanan_table.php?id=' + id,
                type: 'DELETE',
...
<fieldset><legend>Input Data Kejadian Restrain</legend>
    ...
    <form id="formKenyamanan" method="post" action="">
        <table width="100%" border="0" cellpadding="4" cellspacing="2">

            <tr>
                <td width="200"><label for="tgl" id="tgl_kenyamanan_label">Tgl</label></td>
                <td><input type="text" name="tgl" class="required" id="tgl_kenyamanan"> <b>(format:dd-mm-YYYY)</b></td>
            </tr>
