...
$kode	= $_REQUEST['kode'];
$nomr	= $_REQUEST['nomr'];
$sql=mysql_query('select * from m_tarif2012 where kode_tindakan = "'.$kode.'"');
$i = 1;
...
<?php
$sql2=mysql_query('select * from m_tarif2012 where kode_tindakan = "'.$kode.'"');
	$i = 1;
	while($datas = mysql_fetch_array($sql2)){
			echo '<div id="'.$i.'" class="tab_content">';
				$sql_sub = 'select * from m_tarif2012 where kode_tindakan like "'.$datas['kode_tindakan'].'%" and tarif > 0';
