	 ...
	 if($kode_jasa != ""){
			...
			if(mysql_num_rows($get_s) > 0){
				...
				while($dat_s = mysql_fetch_array($get_s)){
					...
					@mysql_query("INSERT INTO t_orderlab(KODE, QTY, IDXDAFTAR, NOMR, TANGGAL, DRPENGIRIM, KDPOLY, KET, RAJAL) VALUES ('$kode_jasa_s', $qty, '$idxdaftar', '$nomr', '$tglreg', '$kddokter', '$kdpoly', '$ket', '1')")or die(mysql_error());
				}
...
header("Location:../../index.php?pesan=$psn&&link=51&menu=5&nomr=".$nomr."&idx=".$idxdaftar);
exit;
