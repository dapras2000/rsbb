<?php session_start();
	...
	if(!empty($_GET['grp'])){
		...
		WHERE LOWER(nama_barang) LIKE LOWER('" . addslashes($_GET['sstring']) . "%') AND (group_barang = '1' OR group_barang = '2') AND farmasi= '".$_GET['farmasi']."' ".$search_unit."
	...
	if ($userquery = mysql_query ($myquery)){
		if (mysql_num_rows ($userquery) > 0){
			?>
			<div style="background: #F5F5F5; border-style: solid; font:Verdana, Geneva, sans-serif; font-size:12px; border-width: 1px; border-color: #000000; overflow:scroll; height:200px" >
			<?php
