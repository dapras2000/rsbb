...
$ip = getRealIpAddr();
mysql_query("INSERT INTO t_resep_detail(NAMA_OBAT, SEDIAAN, ATURAN_PAKAI, JUMLAH, NORESEP, TANGGAL)
			SELECT tmp_cartresep.NAMA_OBAT, tmp_cartresep.SEDIAAN, tmp_cartresep.ATURAN_PAKAI,
  			tmp_cartresep.JUMLAH,'".$maxnorsp."', now()
			FROM tmp_cartresep
			WHERE tmp_cartresep.IP = '".$ip."'");
mysql_query("DELETE FROM tmp_cartresep WHERE tmp_cartresep.IP = '".$ip."'");

header("Location:../../index.php?pesan=Input Sukses&menu=3&link=51&nomr=".$_POST['txtNoMR']."&idx=".$_POST['txtIdxDaftar']);
exit;
?>