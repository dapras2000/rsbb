...
$sql = "SELECT tmp_cartresep.IDXOBAT, tmp_cartresep.NAMA_OBAT, tmp_cartresep.SEDIAAN, tmp_cartresep.ATURAN_PAKAI,
  				tmp_cartresep.JUMLAH
		FROM tmp_cartresep
		WHERE tmp_cartresep.IP = '$ip'";
...
if(empty($_GET['optbarang'])){
	if(empty($_POST['nm_barang']) || empty($_POST['jml_permintaan'])){
		...
		@mysql_query("INSERT INTO tmp_cartresep(NAMA_OBAT, SEDIAAN, ATURAN_PAKAI, JUMLAH, IP)
						VALUES('".$nama_obat."', '".$sediaan."', '".$aturan."', '".$jumlah."', '".$ip."')");
		}
}else{
...
?>
       <tr>
         ...
         <td><?=$data['JUMLAH']?></td>
         <td><a href="#" onclick="javascript: MyAjaxRequest('validbarang','vk/resep/addobatresep.php?optbarang=<?=$data['IDXOBAT']?>&idx=<?=$idxdaftar?>'); return false;" >Batal</a></td>
      </tr>
<?php $i++; } ?>      
   </table>
<? 
...
?>   
   ...
   <form name="savebarang" id="savebarang" action="vk/resep/saveobatresep.php" method="post" >
